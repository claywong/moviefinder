...
UpModule_name=`echo ${module_name:0:1} | tr "[a-z]" "[A-Z]"`${module_name:1}
if [ ! -x "$module_dir/${UpModule_name}Module.php" ]
then
	mv $module_dir/DemoModule.php $module_dir/${UpModule_name}Module.php
	sed -i "" "s/class.*extends/class ${UpModule_name}Module extends/" $module_dir/${UpModule_name}Module.php
fi
